...
def insert(book_Title, book_Author, book_Topic):
    cursor.execute("SELECT * FROM books") # Retrieve the books to find the max id
    results = cursor.fetchall()
    if len(results) == 0: # If there aren't any books
        maxIndex = 0
    else:
        maxIndex = results[-1][0] # Get the largest index
...
def update(id, new_Title, new_Author, new_Topic):
    if pattern.match(id): # Check that the id is an int
        id = pattern.findall(id)[0]
        id = int(id)
        cursor.execute("SELECT * FROM books")
        books = cursor.fetchall()
        for check_id in books: # Check that the id already exists in the table
            if check_id[0] == id:
                cursor.execute("SELECT * FROM books WHERE book_id = {id}".format(id = id))
                book = cursor.fetchall()[0]
                if new_Title == "": # Empty strings should return the old title/author/topic
                    new_Title = book[1]
                if new_Author == "":
                    new_Author = book[2]
                if new_Topic == "":
...
def delete(id):
    if pattern.match(id): # Check that the id is an int
        ...
        cursor.execute("SELECT * FROM books")
        books = cursor.fetchall()
        for check_id in books: # Check that the id exists
            if check_id[0] == id:
...
#----------COMMIT---------------------------
def commit(): # Commit that changes made this session
    connection.commit()
    print("Saving changes")

#----------FORCE INSERT---------------------
def forceInsert(id, book_Title, book_Author, book_Topic):
    # This is a method to INSERT a book with a specific id, this is used to intentionally fill gaps in the table
    cursor.execute("SELECT book_id FROM books")
    books = cursor.fetchall()
    for book in books: # Check that the book at the id doesn't exist.
        if str(book[0]) == id:
